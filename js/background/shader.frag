...
float pattern(vec2 st){
	...
	vec2 q = vec2( fbm(st+ vec2(12.0*(cos(time*0.001),-mouse.y*0.0000003))),
				   fbm(st + vec2(.2,.34)) - (mouse.x*0.000001));
...
void main() {
    ...
    vec3 color = vec3(0.08,0.1,0.1);
    float zoom = 0.066 /ratio;
    ...
    col = mix( col, vec3(0.0,0.2,0.4), 0.6*smoothstep(1.2,1.3,abs(on.z)+abs(on.w)) );
